...
import { neutralizeUntrustedLinks } from './sanitizeOutput';
...
async function run(): Promise<void> {
  ...
  // The notes summarize attacker-writable commit/PR/issue text, so defang any link to a non-github host before the notes are written or rendered. This runs on the model output only, before the attribution is appended, so the attribution's own github link is never touched.
  const safeText = neutralizeUntrustedLinks(text);
  ...
  const outputText = `${safeText.trimEnd()}\n\n${attribution}\n`;
