...
describe('buildReleaseContext', () => {
  ...
  it('covers the whole history for a first release (empty base)', async () => {
    const cfg: Config = {
      owner: 'acme', repo: 'widgets', branch: 'main',
      baseCommit: '', headCommit: 'v1.0.0', // first release: no previous tag
      token: 'token', geminiApiKey: 'gemini-key',
      promptUrl: 'https://example.com/prompt.txt', model: 'gemini-3.1-flash-lite',
      maxLinkedItems: 0, maxReferenceDepth: 2, maxItemLength: 5000,
    };
    ...
    const gh = {
      ...
      listHistory: vi.fn().mockResolvedValue({
        commits: [
          { sha: 'c1', message: 'Initial commit', url: '', author: '@dev', date: '2024-01-01T00:00:00Z' },
          { sha: 'c2', message: 'Second commit', url: '', author: '@dev', date: '2024-01-02T00:00:00Z' },
        ],
        reachedRoot: true,
      }),
      getCommit: vi.fn(),
    ...
    expect((gh.compareCommits as any)).not.toHaveBeenCalled();
    expect((gh.listHistory as any)).toHaveBeenCalledWith('v1.0.0');
    expect(context.range.totalCommits).toBe(2);
    expect(context.range.status).toBe('first-release');
    expect(context.commits).toHaveLength(2);
  });

  it('refuses a first release whose history could not be fully walked', async () => {
    const cfg: Config = {
      owner: 'acme', repo: 'widgets', branch: 'main',
      baseCommit: '', headCommit: 'v1.0.0',
      token: 'token', geminiApiKey: 'gemini-key',
      promptUrl: 'https://example.com/prompt.txt', model: 'gemini-3.1-flash-lite',
      maxLinkedItems: 0, maxReferenceDepth: 2, maxItemLength: 5000,
    };
    ...
    const gh = {
      compareCommits: vi.fn(),
      listHistory: vi.fn().mockResolvedValue({ commits: [], reachedRoot: false }),
      getCommit: vi.fn(),
      getIssueOrPullRequest: vi.fn(),
    } as unknown as GitHubClient;

    await expect(buildReleaseContext(cfg, gh)).rejects.toThrow(/could not be verifiably recovered/i);
  });
});
